...
def find_stopwords_file(lang):
    ...
    candidates = [
        ...
        os.path.join(data_dir, f'stopwords_{lang}.tx'),  # handle truncated extension
    ...
    raise FileNotFoundError(
        f"No stopwords file found for '{lang}' in {data_dir}. "
        f"Tried: {candidates}"
    )
...
def generate(lang):
    ...
    avg_vector = vectors.mean(axis=0, keepdims=True).astype(np.float32)  # shape: (1, 768)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate stopword embedding for a language.')
    parser.add_argument('--lang', required=True, help="Two-character language code (e.g. 'uk')")
